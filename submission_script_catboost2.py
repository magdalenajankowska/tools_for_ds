...
# COVID-19 national case death data: Daily update.
# Retrieved from:
...
# Preprocessing excluding categorical features
...
preprocessor = ColumnTransformer(
    ...
    remainder="passthrough",  # Pass categorical features through unchanged
)
...
# Create the pipeline
pipeline = Pipeline(steps=[("preprocessor", preprocessor), ("model", catboost_model)])

# Prepare transformed categorical indices
# After preprocessing, the categorical features are at the end due to 'passthrough'
cat_indices = list(
    range(len(numerical_columns), len(numerical_columns) + len(categorical_columns))
)

# Update pipeline steps for CatBoost
...
# Retrieve feature names (numerical features are first, then categorical)
preprocessor = best_model.named_steps["preprocessor"]
numerical_features = preprocessor.transformers_[0][2]
categorical_features = categorical_columns  # These should be passed as is
feature_names = list(numerical_features) + categorical_features
